...
class Cross_Attention(nn.Module):
    ...
    def forward(self, y, x):
        ...
        kv = self.kv(x)  # (n_samples, n_patches + 1, 2 * dim)
        ...
        q, k, v = qkv[0], qkv[1], qkv[2]
        #q, k, v = y, x, x
        #print("qkv", q.size(), k.size(), v.size())
        k_t = k.transpose(-2, -1)  # (n_samples, n_heads, head_dim, n_patches + 1)
        #print("k_t", k_t.size())
        dp = (
           q @ k_t
        ...
        attn = self.attn_drop(attn)
        #print("attn_dropout", attn.size())
        weighted_avg = attn @ v  # (n_samples, n_heads, n_patches +1, head_dim)
        #print("weighted_avg1", weighted_avg.size())
        weighted_avg = weighted_avg.transpose(1, 2)  # (n_samples, n_patches + 1, n_heads, head_dim)
        #print("weighted_avg2", weighted_avg.size())
        weighted_avg = weighted_avg.flatten(2)  # (n_samples, n_patches + 1, dim)
        #print("weighted_avg3", weighted_avg.size())
        x = self.proj(weighted_avg)  # (n_samples, n_patches + 1, dim)
        #print("x1", x.size())
        x = self.proj_drop(x)  # (n_samples, n_patches + 1, dim)
        #print("x2", x.size())
        ...
        if prev_state is None:
            ...
            if state_size not in self.zero_tensors:
                ...
                self.zero_tensors[state_size] = (
                    ...
                    torch.zeros(state_size,dtype=input_.dtype).to(input_.device) # recently added the dtype
                )
...
class ConvLSTMSequenceModel(nn.Module):
    ...
    def forward(self, x):#,state):
        ...
        for t in range(seq_len):
          ...
          res =x_t2 + x_t
          x_t2, state = self.recurrent_layer2(res, state)
        #x_t2 = x_t2 +x_t 
