...
def main(args):
    ...
    test_dataset = concatenate_subfolders(join(args.data_path, args.data_folder),
                                           ...
                                           rgb_path,
                                           sequence_length=1,
                                           transform=Compose([CenterCrop(224)]),
    ...
    with torch.no_grad():
        ...
        while idx < N:
            ...
            if args.output_folder and sequence_idx > 1:
                ...
                event_npy_key = join(args.output_folder, "events_npy")
                ensure_dir(event_npy_key)
                np.save(join(event_npy_key, 'event_{:010d}.npy'.format(idx)), input_data)
